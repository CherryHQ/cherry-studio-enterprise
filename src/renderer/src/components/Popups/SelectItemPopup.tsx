...
const PopupContainer: React.FC<PopupContainerProps> = ({ item, resolve }) => {
  ...
  useEffect(() => {
    ...
    const loadPinnedFlows = async () => {
      const setting = await db.settings.get('pinned:flows')
      ...
      if (validPinnedFlows.length !== savedPinnedFlows.length) {
        await db.settings.put({ id: 'pinned:flows', value: validPinnedFlows })
      }

      setPinnedFlows(validPinnedFlows)
    }
    loadPinnedModels()
    loadPinnedFlows()
  }, [providers, chatflows])
  ...
  const togglePinFlow = async (id: string) => {
    const newPinnedFlows = pinnedFlows.includes(id) ? pinnedFlows.filter((id) => id !== id) : [...pinnedFlows, id]

    await db.settings.put({ id: 'pinned:flows', value: newPinnedFlows })
    setPinnedFlows(newPinnedFlows)
  }
        ...
        label: (
          <Item>
            ...
            <PinIcon
              onClick={(e) => {
                e.stopPropagation()
                togglePinModel(getModelUniqId(m))
              }}
        ...
        label: (
          <Item>
            ...
            <PinIcon
              onClick={(e) => {
                e.stopPropagation()
                togglePinModel(getModelUniqId(m.model))
              }}
              isPinned={true}>
              <PushpinOutlined />
            </PinIcon>
  ...
  useEffect(() => {
    if (open && item) {
      setTimeout(() => {
        const id = isModel(item) ? getModelUniqId(item) : item.id
        if (menuItemRefs.current[id]) {
          menuItemRefs.current[id]?.scrollIntoView({ block: 'center', behavior: 'auto' })
        }
  ...
  const selectedKeys = keyboardSelectedId ? [keyboardSelectedId] : item && isModel(item) ? [getModelUniqId(item)] : []
...
export default class SelectItemPopup {
  static hide() {
